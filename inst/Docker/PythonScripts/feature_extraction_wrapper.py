...
def extract_features_from_image(nucleus_m, cell_m, rab, spot, 
                               min_spot_size=8, neighbor_radius=15):
...
def process_condition(condition_path, min_spot_size=8, neighbor_radius=15):
    """Process all images in a condition folder."""
    
    # Load image paths
    nucleus_files = load_images(condition_path, "nucleus_mask")
    cell_files = load_images(condition_path, "cell_mask")
    rab_files = load_images(condition_path, "spots")
    spot_files = load_images(condition_path, "spot_mask")
    ...
    for i in range(n_images):
        ...
        try:
            nucleus_m = imread(nucleus_files[i])
            cell_m = imread(cell_files[i])
            rab = imread(rab_files[i])
            spot = imread(spot_files[i])
...
def main(root_dir, min_spot_size=8, neighbor_radius=15, output_format="csv"):
    ...
    for condition in conditions:
        ...
        features = process_condition(condition_path, min_spot_size, neighbor_radius)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python feature_extraction_wrapper.py <root_dir> [min_spot_size] [neighbor_radius]")
        sys.exit(1)
    ...
    results = main(root_dir, min_spot_size, neighbor_radius)
