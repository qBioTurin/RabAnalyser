...
#here it is performed the feature selection step
...
#plt.savefig('D:/PhD Terzo anno/TESI/immagini/CorrMatrixRab11TRTDLold.svg',transparent=True)
...
# compute the correlation matrix of the reduced features
...
#plt.savefig('D:/PhD Terzo anno/TESI/immagini/CorrMatrixRab11TRTDnew.svg',transparent=True)
plt.savefig('./Plots/allTRTDRab5UMAP.pdf')
#plt.savefig('C:/Users/E14Ge/Desktop/Data_results/allTRTDRab5UMAP.png',dpi=600)


""" UMAP ANALYSIS """

reduced_df.to_excel('./Plots/reduced_df.xlsx',index=False) 

# Standardize the data
scaler = StandardScaler()
data_scaled = scaler.fit_transform(reduced_df)

# Convert scaled data back to a DataFrame with the same column names
scaled_df = pd.DataFrame(data_scaled, columns=reduced_df.columns)

# Save the scaled data to Excel
scaled_df.to_excel('./Plots/reduced_df_scaled.xlsx', index=False)
...
# Save the scaled data to Excel
umap_df.to_excel('./Plots/umap_df.xlsx', index=False)
...
if K_mean_check:
    ...
    plt.savefig('./Plots/ControlRab11UMAP_ref2.pdf')
...
""" CELL PROPORTION PER SUBPOPULATION  """
...
fig.savefig('./Plots/NocoRab11proportion_subpop.pdf')
...
for column in reduced_df.iloc[:,:-1]:
    ...
    scatter = plt.scatter(
        ...
        linewidths=0.005,vmin=vmin,vmax=vmax)# cmpa = 'bwr'
    ...
    #plt.savefig(f'D:/PhD Terzo anno/TESI/immagini/UMAP_Zfyverab5{column}.svg')
    plt.savefig(f'./Plots/UMAP_features2_colControlRef2{column}.pdf')
...
for cluster in reduced_df['Clusters'].unique():
    ...
    feature_fractions = pd.DataFrame({
        'KS > 0': (df_cluster[reduced_df.iloc[:, :-1].columns] > 0).mean(),
        'KS < 0': (df_cluster[reduced_df.iloc[:, :-1].columns] < 0).mean()
    })
    
    # Create the stacked bar plot
    fig, ax = plt.subplots(figsize=(8, 6))
    bottom = np.zeros(len(reduced_df.iloc[:, :-1].columns))
    

    for idx, category in enumerate(['KS > 0', 'KS < 0']):
        ax.bar(reduced_df.iloc[:, :-1].columns, feature_fractions[category], bottom=bottom, color=colors[idx], label=category)
    ...
    plt.savefig(f'./Plots/KSproportionRab11CTRL{cluster}.pdf')
...
for feature in features:
    ...
    #plt.savefig(f'D:/PhD Terzo anno/TESI/immagini/Rab11ViolinCTRL{feature}.svg')
...
# I also print an a dot-plot where the rusults are visualized. Color shades 
...
if n_clusters <= 2: 
    ...
    plt.savefig('./Plots/Statistical_analysis.pdf')
    ...
    scatter = plt.scatter(
        ...
        s=df_significant["-log10(Corrected p-value)"] * 20,  # Scale size for visibility
...
plt.savefig('./Plots/Feat_importanceClusterHM.pdf')
