...
def compute_ks(n):
    ...
    print(f"\rProgress: {percent:.2f} is done\n", end="")
        
    return(KSv)

results = Parallel(n_jobs=-1)(delayed(compute_ks)(n) for n in range(1, 13))

# Convert the list of lists to a DataFrame, setting feature indices as column names
KScontrol = pd.DataFrame(results).T  # Transpose to get the correct shape
