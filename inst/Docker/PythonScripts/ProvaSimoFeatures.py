...
root_dir = str(Path.home() / "Desktop" / "SimoeTealdiDATA" / "Images")
...
for condition_name in condition_folders:
    ...
    #n_images = 1  # to be removed, just for testing
...
######################## Rab SPOTS DISTANCE FROM NUCLEUS CENTROID ######################  

                # print('DISTANCE FROM NUCLEUS')          
                                                
                # distance from nucleus centroid are normalized by the Feret's
                #diameter.
                ...
                # Compute distances of Rab clusters to nucleus centroid 
                Dist = F.NucCd_feret(nuc_centroids[L-1], cell_m_new, spotf)
                
                # Horizontally concatenate with previous features 
                temp = np.hstack((temp, Dist[:,None]))
                
################# Rab SPOTS DISTANCE FROM NUCLEUS OUTLINES ##############

                # print('DISTANCE FROM NUCLEUS OUTLINES')
                # distance from nucleus outlines are normalized by the Feret's
                #diameter.
                
                # Compute distances of Rab clusters to nucleus centroid (custom function)
                Dist = F.NucOutd_feret(mask_nuc, spotf, cell_m_new)
...
################### Rab SPOTS DISTANCE FROM PLASMA MEMBRANE ###################             
                ...
                # distance from cell outlines are normalized by the Feret's
                #diameter.
                ...
                Pdist = F.PMd_feret(cellPerim,spotf,cell_m_new)
...
################### DISTANCE TO THE NEAREST Rab SPOT ##########################
                ...
                # distance to nearest Rab spot are normalized by the Feret's
                #diameters.

                SpotDNN = F.DNN_feret(cell_m_new,spotf)
...
################### NUMBER OF THE NEIGHBOURING Rab SPOTS ######################
    ...
    excel_file = 'Feature_extraction_labels2.xlsx'
